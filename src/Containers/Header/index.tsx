...
export const Header: React.FC<HeaderProps> = ({ dispatch }): JSX.Element => {
  ...
  current =
    current.length > 1
      ? (current.match(locationRegex)?.join(" ") as string)
      : current;
  ...
  const renderMainHeader = (): JSX.Element => {
    if (isSearching) {
      return <SearchInput dispatch={dispatch} />;
    } else {
      return (
        <React.Fragment>
          {isProduct ? (
          ...
          <nav className="Header__desktop">
            {window.location.pathname === "/" ? (
              ...
              <div className="Header__desktop--filter">
                <Filter dispatch={dispatch} isInHeader={true} />
              </div>
            )}
    ...
    <header className="Header">
      {(isMenu || isCart) && window.screen.width < 700 ? (
        <React.Fragment>
          ...
          <h2 className="Header__title">{current}</h2>
        </React.Fragment>
      ) : (
        renderMainHeader()
      )}
