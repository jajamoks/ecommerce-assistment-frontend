...
export function reducer(state: StateInterface, action: ActionType): StateInterface {
  ...
  const getIndex = () => {
    return state.items.findIndex(item => item._id === payload)
  }
  ...
  switch (type) {
    ...
    case "CHANGE_QUANTITY":
      index = state.shoppingCart.findIndex(
        item => item._id === (payload as ChangeQuantityInterface).id
      )
    ...
    case "REMOVE":
      index = getIndex()
      newShoppingCart = state.shoppingCart.filter(product => product._id !== payload)
    ...
    case "INITIAL_ITEM":
      state.item = payload as ItemInterface
      return { ...state, }
